...
# 导入两个版本的模型
from modeling_alrd_llama_fp16 import ALRDLlamaForCausalLMFP16, ALRDLinearFP16
from modeling_alrd_llama_int8 import ALRDLlamaForCausalLMINT8, ALRDLinearINT8, ALRDLinearKeyFP16
...
class BenchmarkResult:
    ...
    avg_decode_latency_ms: float  # ms/token
...
def get_truncation_ranks(config, rank_ratio: float = 0.25) -> Dict[str, int]:
    ...
    Args:
        config: 模型配置
        rank_ratio: 低秩比例 (相对于 hidden_size)
    """
    ranks = {}
...
def load_model_fp16(model_path: str, truncation_ranks: Dict[str, int], device: str = "cuda"):
    """加载 FP16 版本模型"""
    config = AutoConfig.from_pretrained(model_path, trust_remote_code=True)
    model = ALRDLlamaForCausalLMFP16(config)
    model = model.replace_with_alrd(truncation_ranks)
    model = model.to(device).to(torch.float16)
    model.eval()
    return model


def load_model_int8(model_path: str, truncation_ranks: Dict[str, int], device: str = "cuda"):
    """加载 INT8 版本模型"""
    config = AutoConfig.from_pretrained(model_path, trust_remote_code=True)
    model = ALRDLlamaForCausalLMINT8(config)
    model = model.replace_with_alrd(truncation_ranks)
    model = model.to(device).to(torch.float16)
    model.prepare_int8_weights()  # 预量化 INT8 权重
    model.eval()
    return model
...
def benchmark_decode(
    ...
    Returns:
        ...
        throughput: tokens/s
        avg_latency_ms: 平均每 token 延迟 (ms)
    """
...
def run_benchmark(
    ...
    results = []
    config = AutoConfig.from_pretrained(model_path, trust_remote_code=True)
    truncation_ranks = get_truncation_ranks(config, rank_ratio)
    ...
    for model_type in ["FP16", "INT8"]:
        ...
        # 加载模型
        if model_type == "FP16":
            model = load_model_fp16(model_path, truncation_ranks, device)
        else:
            model = load_model_int8(model_path, truncation_ranks, device)
...
def run_quick_benchmark(args):
    """快速测试 (不加载完整模型权重)"""
    print("=" * 100)
    print("Quick Benchmark Mode (Random Weights)")
    print("=" * 100)
    ...
    for model_type in ["FP16", "INT8"]:
        ...
        if model_type == "FP16":
            ...
            model = ALRDLlamaForCausalLMINT8(config)
            model = model.replace_with_alrd(truncation_ranks)
            model.prepare_int8_weights()
        
        model = model.to(device).to(dtype)
        model.eval()
        
        print(f"Model created. Peak memory: {get_peak_memory_gb():.2f} GB")
        ...
        for batch_size in batch_sizes:
            for prefill_len in prefill_lens:
                ...
                try:
                    ...
                    print(f"    Prefill: {prefill_time:.2f}ms, {prefill_throughput:.0f} tokens/s")
                    print(f"    Decode:  {decode_time:.2f}ms, {decode_throughput:.0f} tokens/s")
                    print(f"    Memory:  {peak_memory:.2f} GB")
...
def main():
    ...
    parser.add_argument('--model', type=str, required=True,
                        help='Model path or name (e.g., meta-llama/Llama-3.2-1B)')
    ...
    parser.add_argument('--quick', action='store_true',
                        help='Quick test with random weights (no model loading)')
    parser.add_argument('--load_weights', action='store_true',
                        help='Load actual model weights (slower but more accurate)')
    ...
    if args.quick or not args.load_weights:
        results = run_quick_benchmark(args)
    else:
        batch_sizes = [int(x) for x in args.batch_sizes.split(',')]
        prefill_lens = [int(x) for x in args.prefill_lens.split(',')]
