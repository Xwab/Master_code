...
with Qwen models (no low-rank decomposition, just quantized cache).
...
def demo_basic_usage(model_path: str, k_bits: int = 2, v_bits: int = 2):
    """
    Demo 1: Basic usage with load_qwen_kivi helper
    """
    print("\n" + "=" * 70)
    print("Demo 1: Basic Usage with load_qwen_kivi")
    print("=" * 70)
...
def demo_from_pretrained(model_path: str, k_bits: int = 2, v_bits: int = 2):
    ...
    # Method 1: Create cache manually
    cache = model.create_kivi_cache()
...
def demo_auto_inject(model_path: str, k_bits: int = 2, v_bits: int = 2):
    ...
    model, tokenizer = load_qwen_kivi(model_path, k_bits=k_bits, v_bits=v_bits)
...
def demo_memory_analysis():
    ...
    from modeling_qwen_kivi import KIVICache
...
def main():
    ...
    print("=" * 70)
    print("KIVI Quantized KV Cache Demo for Qwen (No Low-Rank)")
    print("=" * 70)
